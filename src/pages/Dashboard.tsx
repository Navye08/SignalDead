...
export const Dashboard = () => {
  ...
  const accuracy = activeCity ? Number((1.5 + (activeCity.riskScore / 100) * 15).toFixed(1)) : 0;
  const pdop = activeCity ? Number((1.2 + (12 - activeCity.satellites) * 0.3).toFixed(1)) : 0;
  const lastUpdated = new Date().toLocaleTimeString();

  // Dynamic Ambient Page Theme classes based on the risk score
  const isHighRisk = activeCity ? activeCity.currentRisk === 'HIGH RISK' : false;
  const isDegraded = activeCity ? activeCity.currentRisk === 'DEGRADED' : false;
  ...
  return (
    <div className={`space-y-8 p-4 sm:p-6 border transition-all duration-300 relative overflow-hidden rounded-none ${dashboardThemeClass}`}>
      ...
      {activeCity ? (
        ...
        <div className="bg-spaceCard border border-spaceBorder p-12 text-center relative z-10 flex flex-col items-center justify-center min-h-[450px]">
          ...
          <div className="w-16 h-16 border border-spaceBorder bg-spaceBg flex items-center justify-center mb-6 relative">
            ...
            <Satellite className="w-8 h-8 text-spaceSafe animate-pulse" />
          </div>
          ...
          <p className="text-xs font-mono text-gray-400 max-w-md leading-relaxed mb-6">
            GNSS satellite constellation status, ionospheric scintillation indices, and plain-English mission advisories are locked. Use the target station search or GPS locator above to establish a telemetry downlink.
          </p>
          
          <div className="font-mono text-[9px] text-gray-600 border-t border-spaceBorder/50 pt-4 w-full max-w-sm flex justify-between">
            <span>GRID CONSTRAINTS: LOCKED</span>
            <span>SYSTEM ENVELOPE: STANDBY</span>
          </div>
        </div>
      )}
    </div>
